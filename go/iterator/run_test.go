		...
		t.Run(v.name, func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				if got := len(started); got != len(tasks) {
					t.Errorf("%s() concurrently started tasks = %v, want %v", v.name, got, len(tasks))
				}
				close(release)
				<-done
			})
		...
		t.Run(v.name, func(t *testing.T) {
			...
			for _, tt := range tests {
				t.Run(tt.name, func(t *testing.T) {
					...
					got := v.f(slices.Values(tasks))
					if !errors.Is(got, tt.want) {
						t.Errorf("%s() = %v, want %v", v.name, got, tt.want)
					}
					if ran != tt.wantRan {
						t.Errorf("%s() ran %v tasks, want %v", v.name, ran, tt.wantRan)
					}
				})
			}
			...
			t.Run("stops source iteration after error", func(t *testing.T) {
				...
				if err := v.f(tasks); !errors.Is(err, want) {
					t.Errorf("%s() = %v, want %v", v.name, err, want)
				}
				if yielded != 2 {
					t.Errorf("%s() consumed tasks = %v, want %v", v.name, yielded, 2)
				}
			})
		})
	}
